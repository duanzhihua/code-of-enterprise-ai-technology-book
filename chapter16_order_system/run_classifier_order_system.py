...
def open_file(filename, mode='r'):
        ...
        """
        if is_py3:
            return open(filename, mode, encoding='GB18030', errors='ignore')
        else:
            ...
            for line in f:
                try:
                    ...
                    if len(line.strip().split('\t'))> 2 :
                        rejectionid, label, content = line.strip().split('\t')
                        rejection_dict['question']= content
                        rejection_dict['label']= label
                        dicts.append(rejection_dict)
                    else:
                        rejectionid, content = line.strip().split('\t')
                        rejection_dict['question']= content
                        rejection_dict['label']= None
                        dicts.append(rejection_dict)
                except:
                    pass
        return dicts
...
class MyPro(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "data.train.csv")), 'train')

    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "data.val.csv")), 'dev')

    def get_test_examples(self, data_dir,file_name="data.test.csv"):
        return self._create_examples(
